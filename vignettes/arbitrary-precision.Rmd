...
Below we will revisit some of `Rmpfr`'s examples done with `Ryacas` as 
well at demonstrate new ones possible with `Ryacas` but not with `Rmpfr` (e.g. solving systems of linear equations and matrix inverses).
...
The reason for the difference is the near cancellation problem:
Subtraction of nearly identical quantities. This phenomenon is
...
legend("bottomright",
       legend = c(
         parse(text = paste0("e[1]: y == ", as.character(p1))),
         parse(text = paste0("e[2]: y == ", as.character(p2)))),
       lty = c(1, 2),
       cex = 0.7)
```

This example could of course have been illustrated directly in `R` but
it is convenient that `Ryacas` provides exansion of polynomials
