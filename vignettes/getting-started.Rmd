...
There are two interfaces to `yacas`: a low-level ([see vignette](./low-level.html)) and a high-level ([see vignette](./high-level.html)). 
...
Short summaries of often used `yacas` commands and `Ryacas` functions are found 
at the end of the [The low-level interface](./low-level.html) vignette.
...
As you see, there are a lot of nested function call. That can be avoided by using `magrittr`'s pipe `%>%` (automatically available with `Ryacas`) together with the helper function `y_fn()`:
