...
There are two interfaces to `yacas`: a low-level and a high-level. 
The low-level is highly customisable, but also requires more work. 
The high-level is easier for the tasks it can handle, but may also be less computationally efficient.
...
A short summary of often used `yacas` commands are found in the section "`yacas` reference" at the end of this document.
Similarly, a short summary of `Ryacas` functions are also found in the section "`Ryacas` reference" at the end of this document.

### The high-level interface

The high-level interface consists of the main function `yac_symbol()`.
...
As you see, there are a lot of nestings. That can be avoided by using `magrittr`'s pipe `%>%` (automatically available with `Ryacas`) together with the helper function `y_fn()`:
...
If we want the solution without the variable name `x` and the equality symbol `==`, we can use `Ryacas` helper functions `yac_solve_expr()` and `yac_solve_str()` depending on the desired result type:
...
```{r}
"poly == 0" %>% y_fn("Solve", "x")
"poly == 0" %>% y_fn("Solve", "x") %>% yac_solve_str()
```
